...
var (
	configPath = flag.String("config", "configs/order-service.json", "配置文件路径")
)
...
func main() {
	...
	if err := gormDB.AutoMigrate(&order.Order{}); err != nil {
		log.Fatalf("failed to migrate order schema: %v", err)
	}

	// 启动统一的 gRPC 服务模板
	if err := server.RunGRPCServer(cfg, log, func(s *grpc.Server) error {
		orderpb.RegisterOrderServiceServer(s, order.NewGRPCServer(gormDB))
		return nil
	}); err != nil {
		log.Fatalf("order-service exited with error: %v", err)
	}
}
